...
func TestBasicRateLimiting(t *testing.T) {
	rl := NewRateLimiter()
	rl.SetRateLimitConfig("/home", RateLimitConfig{Rate: rate.Every(time.Second), Burst: 1})
	...
	w := performRequest(router, "GET", "/home")
...
func TestDifferentLimitsForEndpoints(t *testing.T) {
	rl := NewRateLimiter()
	rl.SetRateLimitConfig("/test1", RateLimitConfig{Rate: rate.Every(time.Second), Burst: 1})
	rl.SetRateLimitConfig("/test2", RateLimitConfig{Rate: rate.Every(2 * time.Second), Burst: 1})
...
func TestConcurrencyHandling(t *testing.T) {
	rl := NewRateLimiter()
	rl.SetRateLimitConfig("/home", RateLimitConfig{Rate: rate.Every(time.Second), Burst: 5})
