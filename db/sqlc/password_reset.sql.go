...
const deletePasswordResetRequestsByEmail = `-- name: DeletePasswordResetRequestsByEmail :exec
...
func (q *Queries) DeletePasswordResetRequestsByEmail(ctx context.Context, email string) error {
	_, err := q.db.ExecContext(ctx, deletePasswordResetRequestsByEmail, email)
	return err
}

const getPasswordResetRequestByID = `-- name: GetPasswordResetRequestByID :one
SELECT id, user_id, email, token, used, created_at, expires_at FROM password_reset_requests WHERE id = $1 LIMIT 1
`
...
SELECT id, user_id, email, token, used, created_at, expires_at FROM password_reset_requests WHERE token = $1 LIMIT 1
`
